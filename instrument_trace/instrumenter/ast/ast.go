...
func (a instrumenter) Instrument(filename string) ([]byte, error) {
	...
	buf := &bytes.Buffer{}
	err = format.Node(buf, fset, curAST) // ast -> go source code
	if err != nil {
...
func (a instrumenter) addDeferTraceIntoFuncDecls(f *ast.File) {
	for _, decl := range f.Decls {
		fd, ok := decl.(*ast.FuncDecl) // judge whether it is a function declaration
		if ok {
			// inject code to fd
			a.addDeferStmt(fd)
		}
...
func (a instrumenter) addDeferStmt(fd *ast.FuncDecl) (added bool) {
	...
	newList[0] = ds //inject new defer stmt
